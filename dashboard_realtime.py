"""
Real-time MarketTrap dashboard powered by CryptoCompare WebSocket.
Feels like a professional quant terminal.
"""
...
from realtime_feed import start_ws, get_latest_ohlcv

st.set_page_config(page_title="MarketTrap Live", layout="wide")

st.title("MarketTrap – Real-Time Trap Risk Explainer")

# Sidebar: asset selection and API key
with st.sidebar:
    st.header("Settings")
...
def load_anomaly_model():
    return IsolationForestModel.load("models/isolation_forest.pkl")

def run_realtime_pipeline(symbol: str):
    """Fetch live OHLCV from WebSocket buffer and compute trap risk."""
    df = get_latest_ohlcv(symbol, min_ticks=20, window_seconds=60)
...
def render_chart(df: pd.DataFrame, result):
    ...
    fig = make_subplots(
        ...
        subplot_titles=(f"{symbol} Price (Live)", "Trap Risk %"),
...
# Initial load
df, result = run_realtime_pipeline(symbol)
if df is not None and result is not None:
    ...
    st.warning("Waiting for live data… Start the WebSocket feed and ensure enough ticks have arrived.")
